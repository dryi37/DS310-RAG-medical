...
def load_train_data(path: str, add_e5_prefix: bool = True):
    data = []
...
def load_val_data(path: str, add_e5_prefix: bool = True):
    data = []
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)
...
def train(model_name, train_path, val_path, out_dir, batch, epochs, lr):
    ...
    # E5 best practice: prefix query/passsage
    add_e5_prefix = "e5" in model_name.lower()

    train_data = load_train_data(train_path, add_e5_prefix=add_e5_prefix)
    ...
    val_data = load_val_data(val_path, add_e5_prefix=add_e5_prefix)
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune E5-small retriever with val eval")
